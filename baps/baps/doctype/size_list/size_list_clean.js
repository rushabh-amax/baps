// ============================
// Size List - Field-Level Approval System
// ============================
...
//     b1: function(frm, cdt, cdn) { 
//         let row = locals[cdt][cdn];
//         if (row && row._approved_fields && row._approved_fields['b1']) {
//             frappe.show_alert({message: __('B1 field is approved and locked'), indicator: 'red'});
//             return;
//         }
//         calculate_volume(frm, cdt, cdn); 
...
//     b2: function(frm, cdt, cdn) {
//         let row = locals[cdt][cdn];
//         if (row && row._approved_fields && row._approved_fields['b2']) {
//             frappe.show_alert({message: __('B2 field is approved and locked'), indicator: 'red'});
//             return;
//         }
//         if (row && row.b2 >= 12) {
...
//     h1: function(frm, cdt, cdn) { 
//         let row = locals[cdt][cdn];
//         if (row && row._approved_fields && row._approved_fields['h1']) {
//             frappe.show_alert({message: __('H1 field is approved and locked'), indicator: 'red'});
//             return;
//         }
//         calculate_volume(frm, cdt, cdn); 
...
//     h2: function(frm, cdt, cdn) {
//         let row = locals[cdt][cdn];
//         if (row && row._approved_fields && row._approved_fields['h2']) {
//             frappe.show_alert({message: __('H2 field is approved and locked'), indicator: 'red'});
//             return;
//         }
//         if (row && row.h2 >= 12) {
...
function lock_individual_field(frm, row_index, field) {
    setTimeout(() => {
        ...
        if ($field.length) {
            ...
            $field.find('input, select, textarea').prop('readonly', true).prop('disabled', false);
        } else {
            // Fallback: try alternative selector
            let alt_selector = `[data-fieldname="${field}"]`;
            let $alt_field = $(alt_selector).filter(function() {
                return $(this).closest('.grid-row').attr('data-idx') == row_index;
            });
            if ($alt_field.length) {
                $alt_field.addClass('field-approved');
                $alt_field.find('input, select, textarea').prop('readonly', true).prop('disabled', false);
            }
        }
    }, 200);
}
