...
`;
  ...
  frappe.ui.form.GridRow.prototype.make_column = function(df, colsize, txt, ci) {
    let me = this;
    let add_class = ["Text", "Small Text"].indexOf(df.fieldtype) !== -1 ? " grid-overflow-no-ellipsis" : "";
  ...
  frappe.ui.form.Grid.prototype.setup_visible_columns = function() {
    ...
    var total_colsize = 0;
    var fields = this.user_defined_columns && this.user_defined_columns.length > 0 ? this.user_defined_columns : this.editable_fields || this.docfields;
    this.visible_columns = [];
    for (var ci in fields) {
      var _df = fields[ci];
      var df = this.user_defined_columns && this.user_defined_columns.length > 0 ? _df : this.fields_map[_df.fieldname];
...
//# sourceMappingURL=theme.bundle.QADRXLEC.js.map
